...
- 阅读交互：单词解释、句法分析、彩虹拆句、段落翻译、段落概括（支持学生作答后 AI 评价，或 AI 直接输出精准概括）。
- 全文任务：词汇标注、问答题、选择题、判断题、思维导图。
- 流式 AI：SSE 输出、上游并发控制、超时处理、错误映射。
- AI 请求限流：按用户做全局限流，所有大模型相关请求统一 `10 次/分钟`。
...
- 安全控制：CSRF、登录限流、AI 接口按用户限流（每分钟最多 10 次，跨 PM2 worker 共享）、CSP、会话隔离、文件名校验。
